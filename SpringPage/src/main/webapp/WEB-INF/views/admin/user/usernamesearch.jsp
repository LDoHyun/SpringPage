...
<div>
	...
	<table>
     ...
     <hr>
	 <c:forEach items="${userVO}" var="userVO" varStatus="st">
		 <tr>
		 	 <%-- <td align="center">${(pagingVO.curPage-1)*pagingVO.rowPerPage + st.count}</td> --%>
			 <td>${userVO.username}</td>
			 <td><a href="${pageContext.request.contextPath}/admin/user/userview?username=${userVO.username}">${userVO.name}</a></td>
			 <td> ${userVO.phone} - ${userVO.phone2} 
			 	- ${userVO.phone3}</td>
			 <td><fmt:formatDate value="${userVO.birthday}" 
			 				 	 pattern="yyyy년 MM월 dd일" /></td>
             <td><fmt:formatDate value="${userVO.joindate}" 
             					 pattern="yyyy년 MM월 dd일" /></td>
		 </tr>
	 </c:forEach> 
	</table>
