...
function() {
	...
	$("a[id^=boardNum_]")
			.click(
					function(e) {
						...
						var boardNum = e.target.id
								.substring(9); // "boardNum_" 뒤부분 "글번호" 취득

						$
								.ajax({
									url : "${pageContext.request.contextPath}/secured/board/boardDetailJSON.do/boardNum/"
											+ boardNum,
									...
									success : function(
										...
										var boardFile = article.boardFile == null ? "파일없음"
												: article.boardFile;
										...
										if (boardFile == '파일없음')
											...
											$("#boardFile")
													.html(
															"<a href=\"<c:url value='/boardUpload/"+boardFile
	  			   +"'/>\" download >"
																	+ originalFile
																	+ "</a>");
										} //
	...
	// 게시글 팝업(modal) 수정
	$("button[id^=updateContentBtn_]")
			.click(
					function(e) {

						var boardNum = e.target.id
								.substring(17); // "updateContentBtn_" 뒤부분 "글번호" 취득
								...
								.ajax({
									...
									success : function(
										...
										if (article.boardReLev == 0) { // 원글일 경우(답글이 아닐 경우)
											...
											// 파일 필드 추가
											$(
													"form[id=updateForm] #article_content")
													.append(
															fileFldContent); // fileFldContent
											...
											if (boardFile == '파일없음')
												...
												$(
														"#update_boardFile")
														.html(
																"<a href=\"<c:url value='/boardUpload/"
		  					   +boardFile+"'/>\" >"
																		+ boardFile
																		+ "</a>");
	...
	$("button[id^=replySubmitBtn]")
			.click(
					function(e) {
						...
						$
								.ajax({
									url : "${pageContext.request.contextPath}/board/replyWriteBoardProc.do",
									cache : false,
									async : false,
									cache : false,
									contentType : false,
...
<body ng-app="boardFormApp">
	<div>
		...
		<section>
			<div>
				<img
					src="${pageContext.request.contextPath}/pic/coffee_section3.jpg"
		...
		<section id="listForm" style="width: 800px; margin: auto;">
			...
			<c:if test="${not empty articleList && pageVO.listCount > 0}">
					...
					<c:forEach var="article" items="${articleList}" varStatus="st">
						...
						<tr>
							...
							<td><c:choose>
								...
								</c:choose> <%-- <a href="../viewBoard.do/${article.boardNum}">
								${article.boardSubject}</a> --%> <a
								href="../viewBoard.do/${article.boardNum}?page=1"
								id="boardNum_${article.boardNum}" data-toggle="modal"
								data-target="#myModal"> ${article.boardSub} </a></td>

							<td>${article.boardName}</td>
							<td><fmt:formatDate value="${article.boardDate}"
									pattern="yyyy년 MM월 dd일" /> <%-- ${article.boardDate} --%></td>
							<td id="boardReadCount_${article.boardNum}">
								${article.boardReadCount}</td>
						</tr>
				...
				<!-- 게시글 부분 끝 -->
				<h6 align="right">
					<a href="../writeBoardForm.do">게시판글쓰기</a>
				</h6>
				<!-- 페이징(paging) -->
					...
					<ul class="pagination justify-content-center">
						...
						<c:choose>
							<c:when test="${pageVO.page <= 1}">
								...
								<li class="page-item"><a class="page-link"
									href="../boardList.do/1">이전</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="../boardList.do/${pageVO.page - 1}">이전</a></li>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
							<c:when test="${pageVO.page >= pageVO.maxPage}">
								<li class="page-item"><a class="page-link"
									href="../boardList.do/${pageVO.page}">다음</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="../boardList.do/${pageVO.page + 1}">다음</a></li>
							</c:otherwise>
						</c:choose>
